...
min_zoom_level = input("Enter minimum zoom level (default: %s): " %
                       MIN_ZOOM_LEVEL) or MIN_ZOOM_LEVEL
max_zoom_level = input("Enter maximum zoom level (default: %s): " %
                       MAX_ZOOM_LEVEL) or MAX_ZOOM_LEVEL
debug = input("Debugging? y/n (default: %s): " %
              DEBUGGING_FALSE) or DEBUGGING_FALSE
processes = input("Enter number of processes (default: %s): " %
                  PROCESSES) or PROCESSES
resampling = input("Enter resampling method (default: %s): " %
                   RESAMPLING) or RESAMPLING
tms_compatible = input("Is TMS compatible? (default: %s): " %
                       TMSCOMPATIBLE) or TMSCOMPATIBLE
...
if not gpkg_location or not path.exists(gpkg_location):
    ...
    if not load_configurations.is_fs:
        tiles_location = '/vsis3/{0}/{1}'.format(
            load_configurations.bucket, tiles_location)

    tms_compatible = "--tmscompatible" if tms_compatible else ""
    debug = debug.lower()
    verbose = "--verbose" if debug == DEBUGGING_TRUE else ""

    print("Starting export to tiles...")
    for zoom_level in range(int(min_zoom_level), int(max_zoom_level)+1):
        script = 'gdal2tiles.py {0} -e -n -p {1} -r bilinear -z {2} {3} {4} {5} --processes {6} {7}'.format(load_configurations.aws_config, profile,
