...
function save_blob_as_image() {
    ...
    convert $FULL_FILE_NAME "$FILE_NAME.png" && rm $FULL_FILE_NAME
}
GPKG=$GPKG_LOCATION
...
BLOBS=($(sqlite3 $GPKG "select zoom_level, tile_column, tile_row, hex(tile_data) from $GPKG_TABLE_NAME where zoom_level < 5 limit $BATCH_SIZE offset $OFFSET"))
